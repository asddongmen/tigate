...
func TestAdjustScanIntervalDecreaseIgnoresCooldown(t *testing.T) {
	...
	status := newChangefeedStatus(common.NewChangefeedID4Test("default", "test"), 0)
	now := time.Now()
...
func TestGetSyncPointInterval(t *testing.T) {
	t.Parallel()

	disabled := newMockDispatcherInfo(t, 0, common.NewDispatcherID(), 1, 0)
	disabled.enableSyncPoint = false
	disabled.syncPointInterval = 2 * time.Minute
	require.Equal(t, time.Duration(0), getSyncPointInterval(disabled))

	enabled := newMockDispatcherInfo(t, 0, common.NewDispatcherID(), 1, 0)
	enabled.enableSyncPoint = true
	enabled.syncPointInterval = 2 * time.Minute
	require.Equal(t, 2*time.Minute, getSyncPointInterval(enabled))

	invalid := newMockDispatcherInfo(t, 0, common.NewDispatcherID(), 1, 0)
	invalid.enableSyncPoint = true
	invalid.syncPointInterval = 0
	require.Equal(t, time.Duration(0), getSyncPointInterval(invalid))
}
