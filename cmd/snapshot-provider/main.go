...
func main() {
	...
	workers := flag.Int("workers", 1, "maximum concurrent local range workers per job (1-32)")
	flag.Parse()
	...
	controller := provider.New(*root, *binary)
	controller.Workers = *workers
	s := http.Server{Addr: *addr, Handler: controller, ReadHeaderTimeout: 5 * time.Second}
